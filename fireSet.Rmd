...
yTr <- to_categorical(training$FUEL, num_classes = 4)
yTest <- to_categorical(testing$FUEL, num_classes = 4)
...
model = keras_model_sequential() %>%
  layer_flatten() %>% 
  layer_dense(units = 128, activation = "relu",input_shape=(5)) %>%
...
xTr <- as.matrix(training[,3:7])
xTest <- as.matrix(testing[,3:7])
